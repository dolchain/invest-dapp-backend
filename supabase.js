...
const Transfered = async (txHash, from, to, amount) => {
  ...
  try {
    ...
    if (userDetail != null && userDetail != undefined) {
      action = 'deposit';
      if (from == centralWalletAddress) action = 'uninvest';
      const newUserDetail = {
        ...userDetail,
        account_usdc: userDetail?.account_usdc + amount,
        invested_usdc:
          userDetail?.invested_usdc -
          (action == 'deposit' ? 0 : amount + txFee),
        uninvest_usdc:
          userDetail.uninvest_usdc - (action == 'deposit' ? 0 : amount)
      };
  ...
  try {
    ...
    if (userDetail != null && userDetail != undefined) {
      ...
      if (to == centralWalletAddress) action = 'invest';

      const profileData = {
        ...userDetail,
        account_usdc:
          userDetail?.account_usdc -
          amount -
          (action == 'withdraw' ? txFee : 0),
        invested_usdc:
          userDetail?.invested_usdc + (action == 'invest' ? amount : 0)
      };
      const { error } = await supabaseAdmin.from('users').upsert([profileData]);
      if (error) throw error;
    }
  } catch (err) {
    console.log(err);
  ...
  if (action == null) return;
    ...
    console.log(txHash, from, to, amount, action);
  }
  try {
